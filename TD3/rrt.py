...
def RRT(img, img2, start, end, stepSize):
    ...
    while pathFound==False:
        ...
        if directCon and nodeCon:
            ...
            # print("parent_y",node_list[i].parent_y)
            X_Y_points = [(w, u) for w, u in zip(node_list[i].parent_x, node_list[i].parent_y)]
            X_Y_points.append(end)
            # print(X_Y_points)
...
def RRT_Module(image_path = 'world2.png', stepSize = 10, start=[20,20], stop = [450,450], selectPoint=True):
    ...
    img = cv2.imread(image_path,0) # load grayscale maze image
    global img2
    img2 = cv2.imread(image_path) # load colored maze image
    ...
    paths = RRT(img, img2, start, end, stepSize)
    # print("image size: ", img.shape)
    # transforms = transform.TransformData()
    img_shape = img.shape
    transformed_value = transform.transform(paths, img_shape)
    print('the transformed path is: ', transformed_value[0])
    print('the heading angles are in degree: ', transformed_value[1],)
    print('the heading angles are in rad: ', transformed_value[2])
...
if __name__ == '__main__':
    ...
    print('the heading angles are in degree: ', transformed_value[1],)
    print('the heading angles are in rad: ', transformed_value[2])
    print('the the distance that will covered is : ', transformed_value[3])
else:
    node_list = [0]
    img2 = 0
    coordinates = []
